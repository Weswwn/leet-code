var maxProduct = function(nums) {
    ...
    for (let i=1;i<nums.length;i++) {
        ...
        console.log(nums[i] * prevMax, nums[i], nums[i] * prevMin);
        
        curMin = Math.min(nums[i] * prevMin, nums[i], nums[i] * prevMax);
        console.log(nums[i] * prevMin, nums[i], nums[i] * prevMax);

		// updating the prevMax & prevMin, these two may swap locations
        prevMax = Math.max(curMax, curMin);
        prevMin = Math.min(curMin, curMax);

        result = Math.max(curMax, result);
    }
    return result;
}
console.log(maxProduct([-2,3,-2,5,-3,-1]));